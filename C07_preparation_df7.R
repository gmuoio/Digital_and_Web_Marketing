...
df_7_tic_clean_final <- df_7_tic_clean %>%
  ...
  mutate(TIC_DATE_TYP = case_when(
    (TIC_DATE_WEEKDAY %in% c(1,7)) ~ "weekend"
    , (TIC_DATE_HOLIDAY == TRUE) ~ "holiday"
    , (TIC_DATE_WEEKDAY %in% (2:6)) ~ "weekday"
    , TRUE ~ "other"
    )
  )
...
plot_df7_dist_hour <- (
  ggplot(data=df7_dist_hour
    ...
    scale_fill_manual(name='Direction',
                      labels = c("Refund", "Purchase"), values = c("#ffa500", "#1c3d86"))+
    theme_minimal() +
    theme(plot.title = element_text(size=15,hjust = 0.5),
          axis.text.x = element_text(size=10, hjust=1),
          axis.text.y = element_text(size=10, hjust=1),
          legend.text = element_text(size=10),
          legend.title = element_text(size=10)) #-- Centering Title
)
...
## plot aggregate per IMPORTO_LORDO percent
plot_df7_dist_sconto <- ( 
  ggplot(data=df7_dist_importosconto %>%
           filter((SCONTO > -250) & (SCONTO < 250))
...
## compute distribution of the variable average IMPORTO_LORDO and SCONTO by COD_REPARTO
df7_dist_avgimportosconto_cod_rep <- df7_dist_importosconto_cod_rep %>%
  group_by(COD_REPARTO,DIREZIONE) %>%
  summarise(AVG_IMPORTO_LORDO = mean(IMPORTO_LORDO)
            , AVG_SCONTO = mean(SCONTO))
...
## plot distribution of the variable AVG_SCONTO by COD_REPARTO
plot_df7_dist_avgsconto_cod_rep <- (
  ggplot(data=df7_dist_avgimportosconto_cod_rep,
         aes(fill = DIREZIONE,
             x = COD_REPARTO,
             y = AVG_SCONTO)) +      
    geom_bar(stat = "identity") + 
...
## plot distribution of the variable AVG_IMPORTO_LORDO by COD_REPARTO
plot_df7_dist_avgimporto_cod_rep <- (
  ggplot(data=df7_dist_avgimportosconto_cod_rep,
         aes(fill = DIREZIONE,
             x = COD_REPARTO,
             y = AVG_IMPORTO_LORDO)) +      
...
plot_df7_dist_id_articolo <- (
  ggplot(data=df7_dist_id_articolo %>%
           filter(NUM_VENDITE < 300) 
         , aes(x=NUM_VENDITE)) +
...
## explore most sold ID_ARTICOLO by COD_REPARTO
df7_dist_idarticolo_codrep <- df_7_tic_clean_final%>%
  group_by(ID_ARTICOLO,COD_REPARTO) %>%
  summarise(TOT_TICs=n_distinct(ID_SCONTRINO)) %>% 
  ungroup() %>%
  group_by(COD_REPARTO) %>% 
  arrange(desc(TOT_TICs)) %>%
  filter(row_number() == 1) %>%
  ungroup() %>%
  as.data.frame()
...
## compute distribution of the variable IMPORTO_LORDO and SCONTO by ID_CLI
...
## compute average IMPORTO_LORDO and average SCONTO by ID_CLI
...
## plot distribution of the variable IMPORTO_LORDO by ID_CLI
plot_df7_dist_importo_id_cli <- (
  ggplot(data=df7_dist_importosconto_id_cli %>%
         filter((IMPORTO_LORDO < 1000))
         , aes(x=IMPORTO_LORDO)) +
...
## compute days between purchases per client
